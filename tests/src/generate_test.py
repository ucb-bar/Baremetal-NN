...
def generate_test_pattern(op, function, inputs, extra_args=None):
    ...
    func_str = env.from_string("""    {{ op }}(&actual{{ inputs }}{{ extra_args }});\n""").render(
...
test_pattern = [
    ...
    # ("add_inplace", lambda a, b: a + b,                 [("actual", torch.zeros((7, 7), device=device)),   ("b", rand((7, 7)))         ]),
    ...
    # ("fill",        lambda a, v: a.fill_(v),            [("actual", torch.zeros((7, 7), device=device)),   ("v", random.random())      ]),
    ...
    #     [("x", rand((6, 5))), ("1", None), ("w", rand((5))), ("b", torch.zeros((5), device=device)), ("1e-05", None)         ]),
    # ("layer_norm",   lambda x, w, b: torch.nn.functional.layer_norm(x, (x.shape[1], ), w, b, eps=1e-05), 
