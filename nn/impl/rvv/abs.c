...
void NN__abs_i8(size_t n, int8_t *y, size_t incy, int8_t *x, size_t incx) {
  while (n > 0) {
    ...
    __riscv_vsse8_v_i8m1(y, incy, vec_abs_x, vl);
    x += vl;
    y += vl;
    n -= vl;
  }
...
void NN__abs_i16(size_t n, int16_t *y, size_t incy, int16_t *x, size_t incx) {
  while (n > 0) {
    ...
    __riscv_vsse16_v_i16m1(y, incy, vec_abs_x, vl);
    x += vl;
    y += vl;
    n -= vl;
  }
...
void NN__abs_i32(size_t n, int32_t *y, size_t incy, int32_t *x, size_t incx) {
  while (n > 0) {
    ...
    __riscv_vsse32_v_i32m1(y, incy, vec_abs_x, vl);
    x += vl;
    y += vl;
    n -= vl;
  }
...
//     vfloat16m1_t vec_r = __riscv_vfabs_v_f16m1(vec_x, vl);
//     __riscv_vse16_v_f16m1(y, incy, vec_r, vl);
//     x += vl;
//     y += vl;
//     n -= vl;
//   }
...
void NN__abs_f32(size_t n, float *y, size_t incy, float *x, size_t incx) {
  while (n > 0) {
    ...
    vfloat32m1_t vec_r = __riscv_vfabs_v_f32m1(vec_x, vl);
    __riscv_vsse32_v_f32m1(y, incy, vec_r, vl);
    x += vl;
    y += vl;
    n -= vl;
  }
