  ...
  switch (out->ndim) {
    case 1:
      ...
      for (size_t i = 0; i < out->shape[0]; i += 1) {
        ((float16_t *)out->data)[i] = NN_floatToHalf(NN_halfToFloat(((float16_t *)a->data)[i]) + NN_halfToFloat(((float16_t *)b->data)[i]));
      }
      return;
      ...
      for (size_t i = 0; i < out->shape[0]; i += 1) {
        for (size_t j = 0; j < out->shape[1]; j += 1) {
          ...
          ((float16_t *)out->data)[i * out->shape[1] + j]
              = NN_floatToHalf(
                  NN_halfToFloat(((float16_t *)a->data)[a_i * a->shape[1] + a_j])
                + NN_halfToFloat(((float16_t *)b->data)[b_i * b->shape[1] + b_j])
              );
        }
      }
      return;
      ...
      for (size_t i = 0; i < out->shape[0]; i += 1) {
        for (size_t j = 0; j < out->shape[1]; j += 1) {
          for (size_t k = 0; k < out->shape[2]; k += 1) {
            ...
            ((float16_t *)out->data)[i * out->shape[1] * out->shape[2] + j * out->shape[2] + k]
                = NN_floatToHalf(
                    NN_halfToFloat(((float16_t *)a->data)[a_i * a->shape[1] * a->shape[2] + a_j * a->shape[2] + a_k])
                  + NN_halfToFloat(((float16_t *)b->data)[b_i * b->shape[1] * b->shape[2] + b_j * b->shape[2] + b_k])
                );
          }
        }
      }
        ...
        for (size_t c = 0; c < out->shape[1]; c += 1) {
          for (size_t h = 0; h < out->shape[2]; h += 1) {
            for (size_t w = 0; w < out->shape[3]; w += 1) {
              ...
              ((float16_t *)out->data)[n * out->shape[1] * out->shape[2] * out->shape[3] + c * out->shape[2] * out->shape[3] + h * out->shape[3] + w]
                  = NN_floatToHalf(
                      NN_halfToFloat(((float16_t *)a->data)[a_n * a->shape[1] * a->shape[2] * a->shape[3] + a_c * a->shape[2] * a->shape[3] + a_h * a->shape[3] + a_w])
                    + NN_halfToFloat(((float16_t *)b->data)[b_n * b->shape[1] * b->shape[2] * b->shape[3] + b_c * b->shape[2] * b->shape[3] + b_h * b->shape[3] + b_w])
                  );
            }
          }
        }
